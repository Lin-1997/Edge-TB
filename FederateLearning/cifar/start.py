...
def create_client (gpu_index, index, start_data_index, end_data_index):
	os.system ("CUDA_VISIBLE_DEVICES=" + str (gpu_index) + " python client.py -i " + str (
		index) + " --start_data_index=" + str (start_data_index) + " --end_data_index=" + str (end_data_index))


def create_parameter_server (client_num, fraction_ratio):
	os.system (
		"CUDA_VISIBLE_DEVICES=1 python parameter_server.py -n " + str (client_num) + " -c " + str (fraction_ratio))
