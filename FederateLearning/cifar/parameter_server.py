...
def calculate_avg_weight ():
	...
	logging.info ('communication round {}:accuracy={}'.
	              format (communication_round_count,
	                      sess.run (accuracy, feed_dict={xs: test_x, ys: one_hot_test_labels})))
	received_client_count = 0
	...
	selected_client_list = np.random.choice (client_num, int
	(client_num * fraction_each_round), replace=False)
	for selected_client_index in selected_client_list:
		logging.info ("client_{} selected".format (selected_client_index))
		requests.post (
			"http://localhost:" + str (all_addresses [selected_client_index]) +
			"/update_weights_then_training",
			files=file)
		central_weights.seek (0)
		file = {'central_weights': central_weights}
...
def start ():
	logging.info ('communication round 0: accuracy={}'.format (
		sess.run (accuracy, feed_dict={xs: test_x, ys: one_hot_test_labels})))
