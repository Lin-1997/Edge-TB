...
def graph_2d ():
	...
	for i in range (node_number):
		...
		for j in range (i):
			x2 = X_list [j]
			y2 = Y_list [j]
			if x1 - connection_distance <= x2 <= x1 + connection_distance \
					and y1 - connection_distance <= y2 <= y2 + connection_distance:
...
def calculate_equation ():
	global a, b, c
	cal1 = np.mat ([[min_dist ** 2, min_dist, 1], [(min_dist + max_dist) ** 2 / 16,
	                                               (min_dist + max_dist) / 4, 1], [max_dist ** 2, max_dist, 1]])
...
if __name__ == '__main__':
	# we do not consider the unit of bandwidth.
	# the value in bw.txt generated by this file can be decimal,
	# but in net/controller/ctl_run_example.py you can only set an integer bandwidth.
	# you can regard bw.txt as some unit (e.g., 1.953mbps),
	# and set it to the downgrade unit (2000kbps) in ctl_run_example.py.
	parser = argparse.ArgumentParser ()
	parser.add_argument ('-n', '--number', dest='number', required=False,
		type=int, help='number of nodes')
	parser.add_argument ('-p', '--probability', dest='probability', required=False,
		type=float, help='connection probability between nodes, 0~1')
	parser.add_argument ('-m', '--min-bw', dest='min_bw', required=False,
		type=int, help='minimum bandwidth between connected nodes')
	parser.add_argument ('-M', '--max-bw', dest='max_bw', required=False,
		type=int, help='maximum bandwidth between connected nodes')
	args = parser.parse_args ()

	# you can pass parameters through the command line, or modify the code directly.
	# all configurable parameters.
	node_number = args.number if args.number else 4
	connection_probability = args.probability if args.probability else 0.2
	min_bw: int = args.min_bw if args.min_bw else 2
	max_bw: int = args.max_bw if args.max_bw else 5
	# all configurable parameters.
