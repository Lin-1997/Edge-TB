...
# this port number should be the same as the one defined in controller/dml_app/ra_peer.py.
dml_port = 4444
...
def manager(app, net):
    ...
    def route_start():
        for name in pn_to_ip:
            ctl_utils.send_data('GET', '/train', pn_to_ip[name], dml_port)
        for name in en_to_ip:
            ctl_utils.send_data('GET', '/train', en_to_ip[name], en_to_port[name])

        global log_file_path
        # create a folder to save the log files of nodes when finished.
        if log_file_path == '':
    ...
    def route_ok():
        ...
        executor.submit(on_route_ok, name)
        return ''

    def on_route_ok(name):
        global current_round, current_stage, current_step

        ok_nodes.add(name)
        if len(ok_nodes) == node_number:
            print('stage finished by all nodes: ' + current_stage + ' ' + str(current_step))
            ok_nodes.clear()

            if current_stage == 'train':
                current_stage = 'reduce'
                current_step = 1
            elif current_stage == 'reduce':
                if current_step == node_number - 1:
                    current_stage = 'gather'
                    current_step = 1
                else:
                    current_step += 1
            elif current_stage == 'gather':
                if current_step == node_number - 1:
                    current_stage = 'train'
                    current_step = 0
                else:
            ...
            if current_stage == 'train':
                ...
                path = '/send?stage=' + current_stage + '&step=' + str(current_step)

            for name in pn_to_ip:
                ctl_utils.send_data('GET', path, pn_to_ip[name], dml_port)
            for name in en_to_ip:
                ctl_utils.send_data('GET', path, en_to_ip[name], en_to_port[name])

    @app.route('/finish', methods=['POST'])
    def route_finish():
        name = request.args.get('name')
        finished_nodes.add(name)
        if len(finished_nodes) == node_number:
